...
"""
KappaMakerAgent — maker-primary, FIFO-aware, activity-guaranteed engine for subnet 79.
...
class KappaMakerAgent(FinanceSimulationAgent):
    # ------------------------------------------------------------------ setup
    def initialize(self) -> None:
        ...
        bt.logging.info(
            f"[KappaMaker uid={self.uid}] MAKER-PRIMARY FIFO lot={QUOTE_LOT} exch_min={self.exch_min} "
            f"tp={self.tp_bps:.1f}bps inv_cap={MAX_INVENTORY_LOTS}lot/{MAX_INVENTORY_EQUITY_FRAC:.0%}eq "
            f"exit_walk={EXIT_WALK_START_S:.0f}-{giveup_s:.0f}s stop={EXIT_STOP_LOSS_BPS:.0f}bps "
            f"activity_deadline={activity_s:.0f}s rt_window={RT_WINDOW_S / 60:.0f}m "
            f"max_entry_maker_fee={MAX_ENTRY_MAKER_FEE} rt_log={MAIN_VALIDATOR[:8]}"
        )
    ...
    def _ensure_simulation(self, validator: str, simulation_id: str | None) -> None:
        ...
        bt.logging.info(
            f"[KappaMaker uid={self.uid}] new simulation: {validator[:8]} sim_id={simulation_id}"
        )
    ...
    def respond(self, state: MarketSimulationStateUpdate) -> FinanceAgentResponse:
        ...
        for book_id in sorted(self.accounts.keys()):
            ...
            try:
                ...
                bt.logging.warning(f"[KappaMaker uid={self.uid}] step {book_id}: {ex}")
    ...
    def _risk_trim(
        ...
        bt.logging.info(
            f"[KappaMaker uid={self.uid}] RISK-TRIM book={book_id} net={net:+.4f} "
    ...
    def _managed_exit(
        ...
        bt.logging.info(
            f"[KappaMaker uid={self.uid}] MANAGED-EXIT book={book_id} reason={reason} "
    ...
    def _activity_close(
        ...
        if long_q >= self.exch_min:
            ...
            px = round(best_ask * (1.0 + slip), self._price_decimals)
            self._submit_limit(response, book_id, OrderDirection.BUY, q, px, ioc=True, post_only=False)
        bt.logging.info(f"[KappaMaker uid={self.uid}] ACTIVITY-CLOSE book={book_id} net={net:+.4f}")
        return True
    ...
    def _sync_precision(self, price_decimals: int, volume_decimals: int) -> None:
        ...
        bt.logging.info(
            f"[KappaMaker uid={self.uid}] priceDecimals={price_decimals} tick={self._tick} "
    ...
    def _log_rt(
        ...
        bt.logging.info(
            f"[KappaMaker uid={self.uid} RT] book={book_id} close={side} "
