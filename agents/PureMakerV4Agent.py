...
"""
PureMakerV4Agent — all-book two-sided maker for Subnet 79 (τaos). The FINAL PureMaker: a clean
TIGHT-CUT execution engine, meant to drop into AdaptiveRouter's _MakerMode behind its gate
(book-selection / idle / sleep are AR's job — NOT this agent's).

WHAT THIS IS: V1 (our proven-best maker) + the one regime-agnostic improvement (the reprice
cushion). The V-series A/B settled two questions, and this agent bakes in both answers:

  1) EXIT — TIGHT cut BEATS never-cut in the current regime. kappa-3 CUBES the downside (LPM3), so
     the cheapest way to protect kappa is to keep every realized loss SMALL and bounded. The never-
     cut V2/V3 bet (wide 50-60bps stop, hold for reversion) lost: one 55bps stop ≈ 70-100× a 10bps
     cut once cubed, and the current regime is not cleanly reverting, so the rare wide stops dominate
     LPM3. Live: V1 (tight, +0.0012/p79) > V2 (never-cut, −0.0002/p81), and AR's tight-cut maker is
     the only thing actually scoring (+0.023/p23). So V4 uses the TIGHT cut: 10-14bps stop / 150s.

  2) ACTIVITY FLOOR — forcing a round-trip on a flat/dead book is a NET LOSS (V3 was the worst of
     our 3 makers, −69 realized). The validator FREE-DROPS up to ~37.5% of 0-RT books (kappa=None,
     never penalized) and never applies the activity factor to a None book, so force-trading a dead
     book only manufactures a negative-kappa scored book that drags the median. V4 IDLES flat books.

WHAT V4 KEEPS that plain V1 lacks:
  * CUSHION THE REPRICE (REPRICE_KEEP_TICKS=1.5): keep a resting quote unless the desired price moved
    > 1.5 ticks (was tick/2) — quotes REST and get hit instead of churning. Cuts ~0.7→0.45
    cancels/order; regime-agnostic, no directional bet. (AR's maker lacks this — it would benefit.)

QUOTING: inside-on-wide (improve = tick if spread > 2·tick) — the proven choice (V1/V2/AR). Best
  price → fills first → more RT density (needed to clear the kappa lookback gate). The V3 at-touch
  experiment is reverted: unproven, and the lower fill priority hurt density.

BOOK SELECTION — none here: quote ALL books, always. No spread/fee gate, no idle, no streak/backoff
  pause — those are AdaptiveRouter's responsibility; this is the pure execution leg. Standalone (no
  gate) it bleeds the maker fee on thin books in a maker-pays regime; the GATE is what makes it earn.
  Risk is bounded purely on the EXIT side (tight managed-exit / vol-stop / reentry cooldown).

EXIT LEVERS (TIGHT-CUT, = V1):
  * Reduce walks from the profit target toward BREAKEVEN with lot age, never to the touch (a late
    fill nets ~0, never gives away the spread), FLOORED at the vol-scaled stop.
  * Stop = VOL-SCALED BAND clamp(MULT × per-book noise, 10, 14)bps — a TIGHT, bounded cut. Calm books
    cut at the 10bps floor, volatile ones at the 14bps cap; the bounded loss keeps LPM3 small.
  * GIVEUP 150s: IOC-cut a non-reverted lot at ~2.5min (≈ the uid-60 hold median).
  * Small inventory cap (1.5 lots) keeps each forced cut small. We never average into the bag.
...
MECHANICS (per book, each step):
  ...
  * Flat  → IDLE if no edge has formed (no force-seed; the book rests at kappa=None, free-dropped);
            else quote both sides inside-on-wide, gated only by the RT/volume budget.
  * Hold  → ONLY the reducing side (never average in). Reduce walks target→breakeven, stop-floored.
  * Activity → a HELD position that somehow outlives the window is force-closed (deep safety; the
            150s giveup normally closes it first). FLAT books are NOT force-traded.
...
# ---- managed exit: TIGHT CUT, bounded loss (= V1, the proven-best of our makers) ----
# kappa-3 CUBES the downside (LPM3), so the cheapest way to protect kappa is to keep every realized
# loss SMALL and bounded. The never-cut V2/V3 bet (wide 50-60bps stop, hold for reversion) lost: one
# 55bps stop ≈ 70-100× a 10bps cut once cubed, and the current regime isn't cleanly reverting, so the
# rare wide stops dominated LPM3 (V2 −0.0002/p81 < V1 +0.0012/p79; AR's tight-cut maker is the only
# thing scoring, +0.023/p23). So V4 cuts TIGHT: a near, bounded stop + a fast time-cut.
...
REPRICE_KEEP_TICKS = 1.5           # keep a resting quote unless the desired price moved > this many
                                   ...
                                   # the one regime-agnostic win carried over from the V3 experiment.
...
class PureMakerV4Agent(FinanceSimulationAgent):
    ...
    def initialize(self) -> None:
        ...
        bt.logging.info(
            f"[PureMakerV4 uid={self.uid}] PURE-MAKER lot={QUOTE_LOT} exch_min={self.exch_min} "
    ...
    def _ensure_simulation(self, validator: str, simulation_id: str | None) -> None:
        ...
        bt.logging.info(
            f"[PureMakerV4 uid={self.uid}] new simulation: {validator[:8]} sim_id={simulation_id}"
        )
    ...
    def respond(self, state: MarketSimulationStateUpdate) -> FinanceAgentResponse:
        ...
        for book_id in sorted(self.accounts.keys()):
            ...
            try:
                ...
                bt.logging.warning(f"[PureMakerV4 uid={self.uid}] step {book_id}: {ex}")
    ...
    def _step_book(
        ...
        #    books return False here (idle at kappa=None, free-dropped) — V4 does NOT force-seed them.
    ...
    def _risk_trim(
        ...
        bt.logging.info(
            f"[PureMakerV4 uid={self.uid}] RISK-TRIM book={book_id} net={net:+.4f} trim={trim}"
        )
        return True
    ...
    def _managed_exit(
        ...
        if st.exit_miss_count >= 2:
            ...
            bt.logging.info(
                f"[PureMakerV4 uid={self.uid}] {stage} book={book_id} "
                f"miss={st.exit_miss_count} slip={slip*1e4:.0f}bps"
            )
        bt.logging.info(
            f"[PureMakerV4 uid={self.uid}] MANAGED-EXIT book={book_id} reason={reason} "
    ...
    def _activity_close(
        ...
        # V4 — FLAT + stale: LEAVE THE BOOK ALONE (the V3 force-seed is REVERTED). Per the validator
        # (taos/im/validator/reward.py): a 0-round-trip book is kappa=None and FREE-DROPPED from the
        # median (up to max_inactive_books_ratio ≈ 37.5% of books, reward.py:347-366), and the per-book
        # activity factor is NEVER applied to a None book (reward.py:330-332). So "activity=1.0" NEVER
        # required trading every book. Force-seeding a dead/edgeless book converts a free-dropped None
        # into a SCORED book whose forced round-trip is a guaranteed small LOSS (seed crosses + closes)
        # → a real NEGATIVE kappa in the median (the 79% term) + IQR-outlier risk + bled realized PnL
        # (the 21% term). V1/V2 and AdaptiveRouter's _IdleMode all idle flat books for exactly this
        # reason — V3's activity floor was the lone misread. So: do nothing; let the book rest at None
        # (free) and keep quoting passively below. Only a HELD position is force-closed to register a RT.
        ...
        if long_q >= self.exch_min:
            ...
            self._submit_limit(response, book_id, OrderDirection.BUY, q, px, ioc=True, post_only=False,
                               settlement=self._loan_settlement(account))
        bt.logging.info(f"[PureMakerV4 uid={self.uid}] ACTIVITY-CLOSE book={book_id} net={net:+.4f}")
        return True
    ...
    def _desired_quotes(
        ...
        spread = best_ask - best_bid       # inside-on-wide (= V1/V2/AR): step 1 tick inside ONLY when
        improve = tick if spread > 2 * tick else 0.0   # the spread is wide; best price → fills first →
                                           # more RT density. (The V3 at-touch improve=0 is reverted:
                                           # unproven, and lower fill priority starved RT density.)
    ...
    def _sync_precision(self, price_decimals: int, volume_decimals: int) -> None:
        ...
        bt.logging.info(
            f"[PureMakerV4 uid={self.uid}] priceDecimals={price_decimals} tick={self._tick} "
    ...
    def _log_rt(
        ...
        bt.logging.info(
            f"[PureMakerV4 uid={self.uid} RT] book={book_id} close={side} "
