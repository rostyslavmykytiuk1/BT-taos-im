"""
TakerScalperV4Agent — pure-taker round-trip + activity engine for subnet 79 (kappa-tuned).
...
Design (reverse-engineered from the top taker uid184 @ 38.127.44.98):
  ...
  * So V3 trades the dense profit engine on rebated books, drops the kappa-projection open-gate, and
...
MIN_GROSS_TP_BPS = 2.5            # >= SL so the per-RT gross skew is NON-NEGATIVE (kappa cubes downside)
MAX_GROSS_SL_BPS = 12.0           # V4 NO-CUT: catastrophe-only (was V3 2.0). Stops realizing the -2bps scratch-stream (the kappa-downside floor cause) + the SL forced bag-dumps.
...
class TakerScalperV4Agent(FinanceSimulationAgent):
    def initialize(self) -> None:
        ...
        # STOP-LOSS TOGGLE (config, default 12 = no-cut). The bigger regime lever: calm regimes favor
        # the tight cut (SL=2, locks rebate-covered breakevens); adverse/choppy regimes favor no-cut
        # (SL=12, avoids realizing the small-loss stream when positions don't revert). Default no-cut
        # (robust); override via max_gross_sl_bps in AGENT_PARAMS (e.g. 2.0 to A/B the tight cut).
        try:
        ...
        bt.logging.info(
            f"[TakerScalperV4 uid={self.uid}] PURE-TAKER lot={LOT} exch_min={self.exch_min} "
    ...
    def _ensure_simulation(self, validator: str, simulation_id: str | None) -> None:
        ...
        bt.logging.info(
            f"[TakerScalperV4 uid={self.uid}] new simulation: {validator[:8]} sim_id={simulation_id}"
        )
    ...
    def _update_sleep_states(self, validator: str, now: int) -> None:
        ...
        for book_id, r in rates.items():
            ...
            if st.sleeping and r < self.sleep_wake_rate:
                ...
                if self._rt_log_enabled(validator):
                    bt.logging.info(f"[TakerScalperV4 uid={self.uid}] WAKE book={book_id} (fee={r * 1e4:+.2f}bps)")
        ...
        if slots > 0:
            ...
            for r, book_id in payers[:slots]:
                ...
                st.sleeping = True
                if self._rt_log_enabled(validator):
                    bt.logging.info(f"[TakerScalperV4 uid={self.uid}] SLEEP book={book_id} (fee={r * 1e4:+.2f}bps)")
    ...
    def respond(self, state: MarketSimulationStateUpdate) -> FinanceAgentResponse:
        ...
        for book_id in sorted(self.accounts.keys()):
            ...
            try:
                ...
                bt.logging.warning(f"[TakerScalperV4 uid={self.uid}] step {book_id}: {ex}")
    ...
    def _step_book(
        ...
        # is what made V4 try to "close" un-closeable dust → dust-abandon → the orphan-flag freeze.)
    ...
    def _sync_order_size(self, volume_decimals: int) -> None:
        ...
        bt.logging.info(
            f"[TakerScalperV4 uid={self.uid}] volumeDecimals={volume_decimals} "
    ...
    def _log_rt(
        ...
        bt.logging.info(
            f"[TakerScalperV4 uid={self.uid} RT] book={book_id} "
