...
"""
PureMakerV2Agent — all-book two-sided maker for Subnet 79 (τaos).
...
(450s, capped by the 480s activity backstop), wide TP (15bps ≈ uid-145's ~17bps capture). The bet:
...
EXIT LEVERS (V2 values; the V2 banner at top explains the V1→V2 change):
  ...
  a passive reversion. V2 leans into that:
    ...
    * Patient giveup (450s in V2, capped by the 480s activity backstop) + small inventory cap keep
...
MECHANICS (per book, each step):
  ...
  * Flat  → quote both sides inside the touch, gated only by the RT/volume budget (no book gate,
            no streak/backoff pause).
...
#     now walks to breakeven over [30s, 450s], holding near the 15bps target far longer). The 480s
...
EXIT_GIVEUP_S = 450.0              # V2: patient hold (was 150) — capped in practice by the 480s activity backstop
...
class PureMakerV2Agent(FinanceSimulationAgent):
    ...
    def initialize(self) -> None:
        ...
        bt.logging.info(
            f"[PureMakerV2 uid={self.uid}] PURE-MAKER lot={QUOTE_LOT} exch_min={self.exch_min} "
    ...
    def _ensure_simulation(self, validator: str, simulation_id: str | None) -> None:
        ...
        bt.logging.info(
            f"[PureMakerV2 uid={self.uid}] new simulation: {validator[:8]} sim_id={simulation_id}"
        )
    ...
    def respond(self, state: MarketSimulationStateUpdate) -> FinanceAgentResponse:
        ...
        for book_id in sorted(self.accounts.keys()):
            ...
            try:
                ...
                bt.logging.warning(f"[PureMakerV2 uid={self.uid}] step {book_id}: {ex}")
    ...
    def _step_book(
        ...
        #    EVERY book (no book is idled). Deadlines stay ordered: this 480s deadline is shorter than
        #    the validator's 600s decay grace, so the factor never starves.
    ...
    def _risk_trim(
        ...
        bt.logging.info(
            f"[PureMakerV2 uid={self.uid}] RISK-TRIM book={book_id} net={net:+.4f} trim={trim}"
        )
        return True
    ...
    def _managed_exit(
        ...
        if st.exit_miss_count >= 2:
            ...
            bt.logging.info(
                f"[PureMakerV2 uid={self.uid}] {stage} book={book_id} "
                f"miss={st.exit_miss_count} slip={slip*1e4:.0f}bps"
            )
        bt.logging.info(
            f"[PureMakerV2 uid={self.uid}] MANAGED-EXIT book={book_id} reason={reason} "
    ...
    def _activity_close(
        ...
        if long_q >= self.exch_min:
            ...
            self._submit_limit(response, book_id, OrderDirection.BUY, q, px, ioc=True, post_only=False,
                               settlement=self._loan_settlement(account))
        bt.logging.info(f"[PureMakerV2 uid={self.uid}] ACTIVITY-CLOSE book={book_id} net={net:+.4f}")
        return True
    ...
    def _sync_precision(self, price_decimals: int, volume_decimals: int) -> None:
        ...
        bt.logging.info(
            f"[PureMakerV2 uid={self.uid}] priceDecimals={price_decimals} tick={self._tick} "
    ...
    def _log_rt(
        ...
        bt.logging.info(
            f"[PureMakerV2 uid={self.uid} RT] book={book_id} close={side} "
