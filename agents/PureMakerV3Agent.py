...
"""
PureMakerV3Agent — all-book two-sided maker for Subnet 79 (τaos). Execution leg for AdaptiveRouter.

V3 vs V2 (execution quality — the GATE stays OUT; book-selection is AdaptiveRouter's job):
  * QUOTE AT THE TOUCH, not inside (improve=0): join the BBO instead of stepping 1 tick inside.
    Post-only-safe (never crosses on the validator's latency lag → no would-cross rejections) and
    catches less informed/adverse flow than being the single most-aggressive price.
  * CUSHION THE REPRICE (REPRICE_KEEP_TICKS=1.5): keep a resting quote unless the desired price
    moved > 1.5 ticks (was half a tick) — so quotes REST and get hit instead of being cancelled and
    reposted every step. Kills the 0.5-0.8 cancels/order churn; cost = slightly lower fill priority.
  * ACTIVITY FLOOR — guarantee >= 1 completed round-trip per 600s on EVERY book (activity factor
    stays 1.0). The validator sets activity=1.0 only with a closed round-trip in the last 600s; a
    pure maker can't guarantee a passive fill, so when a book is flat AND stale, _activity_close now
    forces a tiny marketable IOC round-trip (seed a ~quote_lot long now; the held branch closes it
    next step). NO gate / NO idle / NO sleep — every book trades. ACTIVITY_DEADLINE_S 480->510 so
    the seed+close lands inside the 600s grace with margin. Cheap/rebate-funded while takers rebate.
  * Exit UNCHANGED from V2 (never-cut wide 50-60bps stop, 450s giveup, 15bps TP).

V2 vs V1 (the A/B): identical EXCEPT the exit. V1 cuts tight (10-14bps stop / 150s giveup / 8bps TP)
and showed NEGATIVE realized skew (ratio 0.63, 11.6% fat losses) — the tight cut manufactures the
loss tail kappa-3 cubes. V2 = HOLD FOR REVERSION: far catastrophe-only stop (50-60bps), long giveup
(450s, capped by the 510s activity backstop), wide TP (15bps ≈ uid-145's ~17bps capture). The bet:
reversion converts most would-be cuts into wins/breakevens, so LPM3 (cubed downside) shrinks net and
kappa rises — like uid-145, which never cuts (~0.02% taker) and scores placement ~40. RISK: if the
regime trends instead of reverting, the far stop bleeds (cubed). Leading indicator = realized skew
ratio from our own RT logs; verify there before trusting kappa.

A maker SPECIALIST: the goal is to be TOP when the market is maker-favorable (rich spreads, mean
reversion). It quotes aggressively (two-sided quotes, high frequency) on EVERY book and
carries NO book gate and NO idle/throttle backoff — risk is bounded purely on the EXIT side.

Design derived from analysis of UID 165 / 145 (top makers) trade history vs validator scoring:

BOOK SELECTION — none: quote ALL 128 books, always.
  No spread/fee gate, no idle, no throttle. Kappa is a sticky ~3h-window metric, so briefly idling a
  book is a no-op and a per-step spread gate just flips on noise; in the thin-spread / maker-cost
  regime a 2.5×fee gate idled ~123/128 books and floored the kappa median. uid-145 (placement 36)
  makes on all 128 books with no gate. The activity backstop keeps every book at activity factor 1.0.

PROFIT MECHANISM on every book:
  Posting passive quotes captures the spread plus the mean-reversion of heavy aggressive flow:
  fill on the way down, exit on the bounce. V2 skew profile is MANY small wins (~15bps TP) + RARE
  large losses (the 50-60bps catastrophe stop), i.e. NOT per-trade positive skew — kappa stays high
  ONLY if reversion is frequent enough that the rare cubed 50-60bps losses don't dominate LPM3. That
  is the explicit bet (and the risk): high win-RATE from holding, vs a fat-but-rare loss tail.
...
KEY FAILURE MODE OF UID 165 (and what we fix):
  ...
  the price trended without reverting. We bound that tail with a vol-scaled stop band, a reduce-price
  floor, and a smaller inventory cap (exit-side only — no entry throttle).

EXIT LEVERS (V2 values; the V2 banner at top explains the V1→V2 change):
  Top makers (uid-60/145) win on KAPPA (consistency) by NEVER taker-cutting a loser — they hold for
  a passive reversion. V2 leans into that:
    * Reduce walks to BREAKEVEN, never the touch — a late fill nets ~0, never gives away the spread.
    * Stop is a VOL-SCALED BAND clamp(MULT×per-book noise, FLOOR, CAP) = 50-60bps in V2 — a FAR,
      catastrophe-only cut. Hold through the normal adverse swing; cut only on a genuine trend.
    * Patient giveup (450s in V2, capped by the 510s activity backstop) + small inventory cap keep
      the (cube-amplified) loss tail bounded even while holding for the bounce.
  [V1 control used the tight version of these: stop band 10-14bps, giveup 150s, TP 8bps.]

MECHANICS (per book, each step):
  prune/kappa/noise → managed exit → risk guard → activity backstop → quote → reconcile.
  (No book gate between risk guard and activity backstop — every book is always active.)
  Managed exit runs FIRST so a stop is never blocked by the risk guard's early return.

  * Managed exit → IOC-cut the held side if underwater >= _stop_bps (vol-scaled band) OR lot age >=
            EXIT_GIVEUP_S (patient backstop). Slippage capped. Bounds each realized loss.
  * Flat  → quote both sides AT the touch (V3: improve=0), gated only by the RT/volume budget
            (no book gate, no streak/backoff pause).
  * Hold  → ONLY the reducing side (never average into the bag). Reduce price walks from the profit
            target toward BREAKEVEN with lot age, FLOORED at the vol-scaled stop so a reduce fill
            never loses more than the IOC cut would; unfilled positions are held for reversion.
  * Activity → every book always closes >= 1 RT per window (activity factor 1.0) — no book is idled.
...
# ---- inventory bounds ----
MAX_INVENTORY_LOTS = 1.5           # trimmed (was 2.0): we now hold reverting positions LONGER and
                                   # the stop band is wider, so size is the lever that keeps the
                                   # (cube-amplified) loss tail bounded — per the uid-60 study.
...
# ---- profit target ----
# V2: TP WIDENED 8 -> 15 bps. The thin-edge maker should capture the REVERSION amplitude (uid-145
# realizes ~17bps gross), not the touch. Paired with the far stop below, this is the hold-for-
# reversion bet: rest near the profit target longer (the reduce walks to breakeven only by the
# now-longer giveup), so a bounce is harvested for ~15bps instead of being cut at -10bps.
# Adaptive floor still applies: max(TP_BPS_BASE, TP_FEE_MULT × maker_fee_bps) covers both fee legs.
TP_BPS_BASE = 15.0
TP_FEE_MULT = 2.0                  # floor = 2× maker_fee (covers both legs + small buffer)
QUOTE_EXPIRY_S = 12.0

# ---- managed exit: HOLD FOR REVERSION, cut only on catastrophe (V2) ----
# V2 is the hold-for-reversion bet (vs V1's tight cut). uid-80 V1 logs showed NEGATIVE skew (ratio
# 0.63, 11.6% fat losses, worst -1.49) — manufactured by the tight 10-14bps stop + 150s giveup
# realizing losses inside the very reversion uid-145 holds for. uid-145 (placement ~40, kappa ~0.07)
# NEVER cuts (~0.02% taker) and wins via reversion. So V2:
#   * STOP widened to a FAR catastrophe-only band 50-60bps (was 10-14): hold THROUGH the normal
#     adverse swing, cut only on a genuine trend. Cube-amplified, but rare — the bet is reversion
#     converts most would-be cuts into wins/breakevens, shrinking LPM3 net.
#   * GIVEUP widened 150 -> 450s: give the position time to revert before any time-cut (the reduce
#     now walks to breakeven over [30s, 450s], holding near the 15bps target far longer). The 510s
#     activity backstop is the true outer cap, so positions still close once per window.
#   * the reduce still walks only to BREAKEVEN, never the touch (see _reduce_price).
# RISK (explicit): if the regime turns TRENDING rather than reverting, the 50-60bps stop bleeds hard
# (cubed). Leading indicator = the realized skew ratio from our own RT logs — re-check after it trades.
EXIT_WALK_START_S = 30.0           # start walking reduce from target toward breakeven after 30s
EXIT_GIVEUP_S = 450.0              # V2: patient hold (was 150) — capped in practice by the 510s activity backstop
EXIT_STOP_LOSS_BPS = 50.0          # V2: FLOOR of the far catastrophe band (was 10) — hold through normal swings
EXIT_STOP_CAP_BPS = 60.0           # V2: CAP of the band (was 14) — bounds the cube tail on volatile books
...
# ---- activity (all books) ----
RT_WINDOW_S = 570.0
ACTIVITY_DEADLINE_S = 510.0        # V3: 480->510. Force a round-trip if a book has none this window.
                                   # Must stay < the validator's 600s activity grace; 510 leaves ~90s
                                   # for retries + the seed's close leg to land in-window (AF2). Also
                                   # gives a never-cut reversion time before the force-close fires.
RT_MAX = 15                        # cap on round-trips per RT_WINDOW per book (anti-overtrade)
FORCE_TRIM_SLIPPAGE_BPS = 5.0

# ---- V3 quoting: at-touch + cushioned reprice ----
REPRICE_KEEP_TICKS = 1.5           # keep a resting quote unless the desired price moved > this many
                                   # ticks (was tick/2). Lets quotes rest & fill instead of churning.
SEED_COOLDOWN_S = 15.0             # min gap between activity-floor seeds on a book — prevents
                                   # re-seeding every step while a seed's fill is in flight. 15s (was
                                   # 30) gives ~6 retries in the ~90s window if a seed misses (AF2).
SEED_CROSS_BPS = 18.0              # AF1: the seed cancels our own resting quotes first, then crosses
                                   # with THIS wide-but-capped slip to clear a thin EXTERNAL spread
                                   # (without a market order's gap risk). 18bps crosses ~any normal
                                   # spread; if it still misses, the next stale step retries.
SEED_MAX_PER_STEP = 16             # AF3: cap activity-floor seeds per respond() so a synchronized
                                   # stale event (e.g. post-restart, all books stale at once) can't
                                   # fire a 128-book marketable burst in one step; the rest seed next steps.
...
class _BookState:
    last_rt_ns: int = 0
    last_cut_ns: int = 0
    last_seed_ns: int = 0                  # V3: last activity-floor seed (rate-limits re-seeding)
    seen_ns: int = 0
...
class PureMakerV3Agent(FinanceSimulationAgent):
    ...
    def initialize(self) -> None:
        ...
        self.seed_cooldown_ns = int(SEED_COOLDOWN_S * _NS)
        ...
        self._seeds_this_step = 0          # AF3 per-step seed budget (reset at the top of respond())

        bt.logging.info(
            f"[PureMakerV3 uid={self.uid}] PURE-MAKER lot={QUOTE_LOT} exch_min={self.exch_min} "
    ...
    def _ensure_simulation(self, validator: str, simulation_id: str | None) -> None:
        ...
        bt.logging.info(
            f"[PureMakerV3 uid={self.uid}] new simulation: {validator[:8]} sim_id={simulation_id}"
        )
    ...
    def respond(self, state: MarketSimulationStateUpdate) -> FinanceAgentResponse:
        ...
        self._seeds_this_step = 0          # AF3: per-step activity-floor seed budget (reset each step)
        now = state.timestamp
        ...
        for book_id in sorted(self.accounts.keys()):
            ...
            try:
                ...
                bt.logging.warning(f"[PureMakerV3 uid={self.uid}] step {book_id}: {ex}")
    ...
    def _step_book(
        ...
        # 3) ACTIVITY BACKSTOP — guarantee >=1 RT per window so the activity factor stays 1.0 on
        #    EVERY book (no book is idled). When holding → IOC-close to register the RT; when flat →
        #    force a tiny IOC round-trip (the V3 activity floor). The 510s deadline is < the
        #    validator's 600s decay grace, with margin for the seed's close leg to land in-window.
        ...
        if (now - activity_ref) >= self.activity_deadline_ns:
            if self._activity_close(response, book_id, account,
                                    inv, net, best_bid, best_ask, vol_dp, now, st):
                return
    ...
    def _risk_trim(
        ...
        bt.logging.info(
            f"[PureMakerV3 uid={self.uid}] RISK-TRIM book={book_id} net={net:+.4f} trim={trim}"
        )
        return True

    # ------------------------------------------------------------------ vol-scaled stop band
    def _stop_bps(self, st: _BookState) -> float:
        """Vol-scaled stop: hold through normal noise, cut only on a genuine adverse move. FLOOR keeps
        calm-book behaviour ≈ the legacy fixed stop; CAP bounds the cube-amplified loss tail on
        volatile books. Regime-robust — it adapts to each book's own oscillation, not the regime."""
    ...
    def _managed_exit(
        ...
        """IOC-cut the whole reducible side when the oldest lot is too old (EXIT_GIVEUP_S, the patient
        reversion backstop) or too underwater (past the vol-scaled stop band). Either path bounds the
        loss; positions inside the band are held for a passive reversion (see _reduce_price)."""
        ...
        if st.exit_miss_count >= 2:
            ...
            bt.logging.info(
                f"[PureMakerV3 uid={self.uid}] {stage} book={book_id} "
                f"miss={st.exit_miss_count} slip={slip*1e4:.0f}bps"
            )
        bt.logging.info(
            f"[PureMakerV3 uid={self.uid}] MANAGED-EXIT book={book_id} reason={reason} "
    ...
    def _activity_close(
        ...
        # V3 ACTIVITY FLOOR — flat AND stale (we only reach here past ACTIVITY_DEADLINE with no
        # round-trip this window). A pure maker can't guarantee a passive fill, so to hold the book's
        # activity factor at 1.0 we FORCE a tiny round-trip: a minimal marketable IOC BUY (taker)
        # opens a ~quote_lot long now; next step the book is holding and the held branch below
        # IOC-closes it → the round-trip registers inside the 600s window. Use quote_lot (not
        # exch_min) so the held lot stays >= exch_min after the fee shave (same reason QUOTE_LOT >
        # exch_min). Leave resting quotes in place (they may still fill). Cheap/rebate-funded while
        # takers are rebated; the only non-passive action in the agent. No gate / no idle / no sleep.
        if long_q < self.exch_min and short_q < self.exch_min:
            # Rate-limit (AF2): don't re-seed while a prior seed's fill may still be in flight.
            if now - st.last_seed_ns < self.seed_cooldown_ns:
                return False
            # Per-step cap (AF3): a synchronized stale event (e.g. post-restart) must not fire a
            # marketable burst across all 128 books in one step; over-budget books seed next steps.
            if self._seeds_this_step >= SEED_MAX_PER_STEP:
                return False
            # AF1: CANCEL our own resting quotes FIRST, then cross with a wide-but-capped slip. Without
            # the cancel, the seed first meets our own inside ask; STP (CANCEL_OLDEST) cancels that ask
            # and continues — but with a tight slip the next EXTERNAL ask is beyond maxPrice on a thin
            # book, so the IOC exits unfilled (no RT — exactly the floor's target population). Cancelling
            # clears our tick up front and SEED_CROSS_BPS widens maxPrice enough to clear a thin external
            # spread without a market order's gap risk. If it still misses, "flat next step" retries.
            slip = SEED_CROSS_BPS / 1e4
            buy_px = best_ask * (1.0 + slip)
            q = round(self.quote_lot, vol_dp)
            if buy_px <= 0 or q < self.exch_min or self._avail(account.quote_balance) < q * buy_px:
                return False
            st.last_seed_ns = now
            self._seeds_this_step += 1
            self._cancel_all(response, account, book_id)
            px = round(buy_px, self._price_decimals)
            self._submit_limit(response, book_id, OrderDirection.BUY, q, px, ioc=True, post_only=False)
            bt.logging.info(
                f"[PureMakerV3 uid={self.uid}] ACTIVITY-SEED book={book_id} q={q} @~{px} "
                f"(force RT, cross {SEED_CROSS_BPS:.0f}bps)"
            )
            return True
        ...
        if long_q >= self.exch_min:
            ...
            self._submit_limit(response, book_id, OrderDirection.BUY, q, px, ioc=True, post_only=False,
                               settlement=self._loan_settlement(account))
        bt.logging.info(f"[PureMakerV3 uid={self.uid}] ACTIVITY-CLOSE book={book_id} net={net:+.4f}")
        return True
    ...
    def _desired_quotes(
        ...
        improve = 0.0                      # V3: quote AT the touch (join the BBO), never inside it —
                                           # post-only-safe + lower adverse selection (see docstring).
    ...
    def _sync_precision(self, price_decimals: int, volume_decimals: int) -> None:
        ...
        bt.logging.info(
            f"[PureMakerV3 uid={self.uid}] priceDecimals={price_decimals} tick={self._tick} "
    ...
    def _log_rt(
        ...
        bt.logging.info(
            f"[PureMakerV3 uid={self.uid} RT] book={book_id} close={side} "
