"""
TakerScalperNCAgent — EXPERIMENT fork of TakerScalperAgent (don't touch the live version).
Only change vs TakerScalperAgent: the activity backstop forces ~5 RTs per 3h window (comfortably above
the min_realized_observations=3 SCORED floor, 2-RT margin) instead of ~18 (old 500s cadence). Tests the owner's point
that activity=1.0 needs only a majority of books active over 3h, so forced 10-min churn was wasteful.
Everything else (tight rebate-cushioned scalp, SL=4, single-open) is identical — taker losses are
rebate-cushioned to net ~0 so the taker has no never-cut problem; this fork is purely the cadence test.
...
Design (reverse-engineered from the top taker uid184 @ 38.127.44.98):
  ...
  * SLEEP is FEE-ONLY with hysteresis: a book SLEEPS when its taker fee is POSITIVE (it pays to take)
    ...
    inactive injects hard 0.0s into the validator median — catastrophic). no_sleep=1 disables sleep
    entirely (trade all books — the uid184 null hypothesis, for A/B). NB: the validator's real aggregation
...
ACTIVITY_DEADLINE_S = 2160.0       # = KAPPA_RT_HISTORY_S / (KAPPA_MIN_OBS + 2)
...
class TakerScalperNCAgent(FinanceSimulationAgent):
    def initialize(self) -> None:
        ...
        # SLEEP TOGGLE (config, default OFF). Validated 2026-06-25 by per-book counterfactual: sleep
        # correctly skips fee-positive books (38/38 were negative-kappa in no-sleep uid192), but the
        # benefit is tiny (+0.0002 internal / +0.0008 scored) and ONLY in fee-adverse regimes; in calm
        # regimes it sacrifices breadth (the kappa driver) for nothing. So DEFAULT OFF (no_sleep=1);
        # flip ON (no_sleep=0 in AGENT_PARAMS) only in a confirmed fee-adverse regime. ParseKwargs
        # coerces "1" -> float 1.0, so coerce numerically first, then fall back to text flags.
        _ns_raw = getattr(self.config, "no_sleep", 1)
        try:
            self.no_sleep = bool(float(_ns_raw))
        except (TypeError, ValueError):
            self.no_sleep = str(_ns_raw).strip().lower() in ("true", "yes", "on")

        # STOP-LOSS TOGGLE (config, default 4 = calm-regime cut). The bigger regime lever: calm regimes
        # favor the cut (SL=4, rebate-cushioned ~breakeven — locks rebate-covered exits WITHOUT the
        # false-triggering that SL=2 caused via our own market-open impact); adverse/choppy regimes
        # favor no-cut (SL=12, avoids realizing the small-loss stream when positions don't revert).
        # Override via max_gross_sl_bps in AGENT_PARAMS (e.g. 12 for no-cut, 2 to A/B the ultra-tight cut).
        try:
            self.max_gross_sl_bps = float(getattr(self.config, "max_gross_sl_bps", MAX_GROSS_SL_BPS))
        except (TypeError, ValueError):
        ...
        bt.logging.info(
            f"[TakerScalperNC uid={self.uid}] PURE-TAKER lot={LOT} exch_min={self.exch_min} "
            f"hold={MIN_HOLD_S}-{max_hold_s:.1f}s tp={MIN_GROSS_TP_BPS}bps sl={self.max_gross_sl_bps}bps "
            ...
            f"sleep={'OFF' if self.no_sleep else 'fee'}(sleep fee>0, wake fee<{SLEEP_WAKE_FEE_BPS}bps, "
    ...
    def _ensure_simulation(self, validator: str, simulation_id: str | None) -> None:
        ...
        bt.logging.info(
            f"[TakerScalperNC uid={self.uid}] new simulation: {validator[:8]} sim_id={simulation_id}"
        )
    ...
    def _update_sleep_states(self, validator: str, now: int) -> None:
        ...
        for book_id, r in rates.items():
            ...
            if st.sleeping and r < self.sleep_wake_rate:
                ...
                if self._rt_log_enabled(validator):
                    bt.logging.info(f"[TakerScalperNC uid={self.uid}] WAKE book={book_id} (fee={r * 1e4:+.2f}bps)")
        ...
        if slots > 0:
            ...
            for r, book_id in payers[:slots]:
                ...
                st.sleeping = True
                if self._rt_log_enabled(validator):
                    bt.logging.info(f"[TakerScalperNC uid={self.uid}] SLEEP book={book_id} (fee={r * 1e4:+.2f}bps)")
    ...
    def respond(self, state: MarketSimulationStateUpdate) -> FinanceAgentResponse:
        ...
        for book_id in sorted(self.accounts.keys()):
            ...
            try:
                ...
                bt.logging.warning(f"[TakerScalperNC uid={self.uid}] step {book_id}: {ex}")
    ...
    def _sync_order_size(self, volume_decimals: int) -> None:
        ...
        bt.logging.info(
            f"[TakerScalperNC uid={self.uid}] volumeDecimals={volume_decimals} "
    ...
    def _log_rt(
        ...
        bt.logging.info(
            f"[TakerScalperNC uid={self.uid} RT] book={book_id} "
