"""
TakerScalperV3Agent — pure-taker round-trip + activity engine for subnet 79 (kappa-tuned).
...
MAX_GROSS_SL_BPS = 2.0            # EXIT stop, tight; SL <= TP keeps wins >= losses (positive skew)
...
class TakerScalperV3Agent(FinanceSimulationAgent):
    def initialize(self) -> None:
        ...
        # A/B knob: no_sleep=1 in AGENT_PARAMS disables sleep entirely (the uid184 null hypothesis —
        # trade every book). NOTE: ParseKwargs (taos/common/config) coerces "1" -> float 1.0, so coerce
        # numerically first, then fall back to text flags. Default off.
        _ns_raw = getattr(self.config, "no_sleep", 0)
        try:
        ...
        bt.logging.info(
            f"[TakerScalperV3 uid={self.uid}] PURE-TAKER lot={LOT} exch_min={self.exch_min} "
    ...
    def _ensure_simulation(self, validator: str, simulation_id: str | None) -> None:
        ...
        bt.logging.info(
            f"[TakerScalperV3 uid={self.uid}] new simulation: {validator[:8]} sim_id={simulation_id}"
        )
    ...
    def _update_sleep_states(self, validator: str, now: int) -> None:
        ...
        for book_id, r in rates.items():
            ...
            if st.sleeping and r < self.sleep_wake_rate:
                ...
                if self._rt_log_enabled(validator):
                    bt.logging.info(f"[TakerScalperV3 uid={self.uid}] WAKE book={book_id} (fee={r * 1e4:+.2f}bps)")
        ...
        if slots > 0:
            ...
            for r, book_id in payers[:slots]:
                ...
                st.sleeping = True
                if self._rt_log_enabled(validator):
                    bt.logging.info(f"[TakerScalperV3 uid={self.uid}] SLEEP book={book_id} (fee={r * 1e4:+.2f}bps)")
    ...
    def respond(self, state: MarketSimulationStateUpdate) -> FinanceAgentResponse:
        ...
        for book_id in sorted(self.accounts.keys()):
            ...
            try:
                ...
                bt.logging.warning(f"[TakerScalperV3 uid={self.uid}] step {book_id}: {ex}")
    ...
    def _sync_order_size(self, volume_decimals: int) -> None:
        ...
        bt.logging.info(
            f"[TakerScalperV3 uid={self.uid}] volumeDecimals={volume_decimals} "
    ...
    def _log_rt(
        ...
        bt.logging.info(
            f"[TakerScalperV3 uid={self.uid} RT] book={book_id} "
