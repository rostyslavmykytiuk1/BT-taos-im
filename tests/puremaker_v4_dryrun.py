"""
Dry-run for the FINAL PureMakerV4Agent (tight-cut). NO network / validator / deploy. Unit-tests the
behaviors the final spec promises, calling agent methods directly with mock state:
  ...
  H. CUSHIONED REPRICE (1.5 ticks): keep within 1.5 ticks, cancel beyond.
...
spec = importlib.util.spec_from_file_location("pmv4", REPO / "agents" / "PureMakerV4Agent.py")
mod = importlib.util.module_from_spec(spec)
spec.loader.exec_module(mod)
V4, Inv, OD, TIF = mod.PureMakerV4Agent, mod._Inv, mod.OrderDirection, mod.TimeInForce
PASS, FAIL = [], []
...
print("\n[H] CUSHIONED REPRICE (REPRICE_KEEP_TICKS=1.5)")
a = agent()
r = Resp()  # within 1.5 ticks (1 tick away) -> KEEP
a._reconcile_quotes(r, acct(base_free=10, orders=[NS(side=0, price=300.00, quantity=0.26, id=1)]),
                    9, {OD.BUY: (300.01, 0.26)})
check(r.cancels == [] and r.orders == [], "resting quote 1 tick from desired -> KEPT")
r2 = Resp()  # beyond 1.5 ticks (3 ticks away) -> CANCEL
a._reconcile_quotes(r2, acct(base_free=10, orders=[NS(side=0, price=300.00, quantity=0.26, id=2)]),
                    9, {OD.BUY: (300.03, 0.26)})
check(bool(r2.cancels) and r2.cancels[0][1] == [2], "resting quote 3 ticks from desired -> CANCELLED")
